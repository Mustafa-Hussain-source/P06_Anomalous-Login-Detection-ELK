from __future__ import annotations

import json
import os
import sqlite3
import sys
from datetime import datetime
from pathlib import Path


def _read_alert() -> dict:
    raw = sys.stdin.read().strip()
    if not raw:
        return {}
    try:
        return json.loads(raw)
    except json.JSONDecodeError:
        return {}


def _extract_user_id(alert: dict) -> int | None:
    for key in ("user_id", "uid", "id"):
        value = alert.get(key)
        if isinstance(value, int):
            return value
    data = alert.get("data") or {}
    for key in ("user_id", "uid", "id"):
        value = data.get(key)
        if isinstance(value, int):
            return value
    return None


def _get_db_path() -> Path:
    env_value = os.getenv("ALDS_DB_PATH")
    if env_value:
        return Path(env_value)
    return Path(__file__).resolve().parents[2] / "alds.db"


def main() -> int:
    alert = _read_alert()
    user_id = _extract_user_id(alert)
    if user_id is None:
        print(json.dumps({"status": "fail", "reason": "user_id missing"}))
        return 1

    db_path = _get_db_path()
    conn = sqlite3.connect(db_path)
    try:
        conn.execute("DELETE FROM active_sessions WHERE user_id = ?", (user_id,))
        conn.execute(
            """
            INSERT INTO mitigation_log (uc_id, target_identifier, action, status, timestamp)
            VALUES (?, ?, ?, ?, ?)
            """,
            ("UC-014", str(user_id), "session_kill", "success", datetime.now().isoformat()),
        )
        conn.commit()
        print(json.dumps({"status": "success", "action": "session_kill", "user_id": user_id}))
        return 0
    finally:
        conn.close()


if __name__ == "__main__":
    raise SystemExit(main())
